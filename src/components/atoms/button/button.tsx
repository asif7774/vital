...
const Button = forwardRef<HTMLButtonElement, ButtonProps>(
  (
    ...
    const baseStyles = "inline-flex items-center justify-center font-medium rounded-md transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed";
    // Default loader icon
    const LoadingIcon = () => (
      <SvgIcon name="loader" className="animate-spin" width="16" height="16" aria-hidden={true} />
    );

    const IconElement = isLoading ? LoadingIcon : (icon ? () => <SvgIcon name={icon} width="18" height="18" aria-hidden={true} /> : null);
    
    const iconSpacing = children && size !== "icon" ? "gap-2" : "";
    
    const combinedClassName = `${baseStyles} ${variantStyles[variant]} ${sizeStyles[size]} ${iconSpacing} ${className}`.trim();
